# KineticsForge V2 Kaggle One-Cell Runner

Attach `kineticsforge_v2_input.zip` as a Kaggle Dataset input, then paste the cell below into a Kaggle notebook.

```python
import os, sys, json, shutil, subprocess, zipfile, pathlib

ROOT = pathlib.Path("/kaggle/working/kineticsforge_v2")
INPUTS = list(pathlib.Path("/kaggle/input").rglob("kineticsforge_v2_input.zip"))
if not INPUTS:
    raise FileNotFoundError("Attach kineticsforge_v2_input.zip as a Kaggle Dataset input.")
if ROOT.exists():
    shutil.rmtree(ROOT)
