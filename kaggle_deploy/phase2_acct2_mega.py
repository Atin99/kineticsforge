"""ACCT2 MEGA-CELL: Trains 3 models sequentially on real data
M5: BMS Pack Risk (cell failure prediction via TGN)
M6: RUL Predictor (remaining useful life regression)
M7: Anomaly Detector (autoencoder for abnormal degradation)
Budget: 20 hrs
"""
import torch, torch.nn as nn, numpy as np, json, time, os, glob
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
SLUG = "kineticsforge-realdata-acct2a"
...
def find_data_root():
    ...
    # List everything first for debugging
...
def train_model(name, model, train_loader, val_loader, epochs, lr, save_name):
    ...
    for ep in range(start_ep, epochs):
        if not time_ok():
            ...
            print(f"  TIME LIMIT {name} ep {ep}"); return False
    ...
    print(f"  DONE {name} best={best:.6f}"); return True

# ── MODEL 5: BMS PACK RISK (TGN) ────────────────────────────
...
# ── MODEL 7: ANOMALY DETECTOR (AUTOENCODER) ─────────────────
        ...
        with torch.amp.autocast("cuda",enabled=DEVICE=="cuda"):
            ...
            for i in range(len(lengths)):
                ...
                for j in range(WINDOW, L, max(1,(L-WINDOW)//6)):
                    ...
                    recon, _ = self(inp.unsqueeze(0))
                    tgt = torch.cat([h,f],dim=-1)
...
print(f"train={len(train_ds)} val={len(val_ds)} b/ep={len(train_loader)}")
