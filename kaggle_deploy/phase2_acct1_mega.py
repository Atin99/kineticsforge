"""ACCT1 MEGA-CELL: Trains 4 models sequentially on real data
M1: Cathode UDE (degradation curve prediction)
M2: SOH Estimator (state of health from recent cycles)
M3: Cycle Life Classifier (predict total lifetime)
M4: Fade Rate Predictor (degradation speed)
Budget: 20 hrs (will fit in ~8 hrs total)
"""
import torch, torch.nn as nn, numpy as np, json, time, os, glob
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
SLUG = "kineticsforge-realdata-acct1a"
INPUT = f"/kaggle/input/{SLUG}"
CKPT = "/kaggle/working/checkpoints"
os.makedirs(CKPT, exist_ok=True)
t0 = time.time(); TL = 11.0*3600.0
LOG_EVERY = 5; MAX_SEQ = 600; N_FEAT = 27; COND_DIM = 5

# ── AUTO-FIND DATA (recursive scan for train/*.npz) ──────────
def find_data_root():
    base = "/kaggle/input"
...
# ── TASK TRACKER ─────────────────────────────────────────────
def load_tracker():
    p = os.path.join(CKPT,"tracker.json")
    if os.path.exists(p):
        with open(p) as f: return json.load(f)
    for d in [INPUT]:
        tp = os.path.join(d,"tracker.json")
...
def train_model(name, model, train_loader, val_loader, epochs, lr, save_name):
    ...
    if not os.path.exists(ckp):
        ckp = os.path.join(INPUT, f"{save_name}_resume.pt")
    ...
    print(f"  training {name}: {epochs} epochs, lr={lr}, batches/ep={total_b}")
    ...
    for ep in range(start_ep, epochs):
        if not time_ok():
            ...
            print(f"  TIME LIMIT {name} at epoch {ep}"); return False
        model.train(); ep_loss=0; nb=0
        for bi, batch in enumerate(train_loader):
            if not time_ok():
                torch.save({"model":model.state_dict(),"opt":opt.state_dict(),"sched":sched.state_dict(),"scaler":scaler.state_dict(),"epoch":ep,"best":best}, os.path.join(CKPT,f"{save_name}_resume.pt"))
                tracker["epoch"]=ep; tracker["best"]=best; save_tracker(tracker)
                print(f"  TIME LIMIT {name} mid e={ep}"); return False
            ...
            if (bi+1)%LOG_EVERY==0 or bi==total_b-1:
                print(json.dumps({"M":name,"e":ep+1,"b":f"{bi+1}/{total_b}","loss":round(bl,6),"avg":round(ep_loss/max(nb,1),6),"best":round(best,6),"min":round((time.time()-t0)/60,1),"rem_hr":round(max(0,TL-(time.time()-t0))/3600,2)}))
        sched.step()
        if (ep+1)%10==0:
            model.eval(); vl=0; vn=0
            with torch.no_grad():
                for vb in val_loader:
                    vl += float(model.training_step(vb).detach()); vn+=1
            vm = vl/max(vn,1)
            print(json.dumps({"M":name,"EVAL":ep+1,"val":round(vm,6),"best":round(best,6)}))
            if vm < best:
                best = vm
                torch.save(model.state_dict(), os.path.join(CKPT,f"{save_name}_best.pt"))
                print(f"  NEW BEST {name} val={vm:.6f}")
        if (ep+1)%25==0:
    ...
    tracker["done"].append(name); tracker["current"]=None; tracker["epoch"]=0; tracker["best"]=999
    save_tracker(tracker)
    torch.save(model.state_dict(), os.path.join(CKPT,f"{save_name}_final.pt"))
    print(f"  DONE {name} best={best:.6f}")
    return True
...
class CathodeUDE(nn.Module):
    ...
    def forward(self, t, state, z, fz):
        ...
        g = self.gate(inp); nn_out = self.neural(inp)
        ...
        with torch.amp.autocast("cuda",enabled=DEVICE=="cuda"):
            ...
            for i in range(len(lengths)):
                ...
                for j in range(WINDOW, L, max(1,(L-WINDOW)//10)):
                    h = cap[i,j-WINDOW:j]
                    f = feats[i,min(j,feats.shape[1]-1)]
                    soh = cap[i,j]
...
print(f"train={len(train_ds)} val={len(val_ds)} test={len(test_ds)} batches/ep={len(train_loader)}")
...
for name, mk_model, epochs, lr, save_name in tasks:
    if not time_ok():
        print(f"TIME LIMIT reached before {name}"); break
    print(f"\n{'='*60}\nSTARTING {name}\n{'='*60}")
    model = mk_model()
    result = train_model(name, model, train_loader, val_loader, epochs, lr, save_name)
    if result == False:
        break

# Final test eval for completed models
tracker = load_tracker()
print(f"\n{'='*60}\nFINAL SUMMARY\n{'='*60}")
print(f"Completed: {tracker['done']}")
print(f"Current: {tracker.get('current','none')} at epoch {tracker.get('epoch',0)}")
for f in os.listdir(CKPT):
    print(f"  {f}: {os.path.getsize(os.path.join(CKPT,f))} bytes")
